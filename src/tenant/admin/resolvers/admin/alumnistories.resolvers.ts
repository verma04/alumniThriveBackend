...
const alumniStoriesResolvers = {
  Query: {
    async getAllAlumniStoriesCategory(_: any, { input }: any, context: any) {
      try {
    ...
    async getAllAlumniStories(_: any, { input }: any, context: any) {
      try {
        ...
        const stories = await db.query.alumniStory.findMany({
          ...
          orderBy: (alumniStory, { desc }) => [desc(alumniStory.createdAt)],
          with: {
            category: true,
            user: {
              with: {
    ...
    async getAllApprovedAlumniStories(_: any, { input }: any, context: any) {
      try {
        const data = await checkAuth(context);
    ...
    async getAllApprovedRequestedStories(_: any, { input }: any, context: any) {
      try {
        const data = await checkAuth(context);
  ...
  Mutation: {
    ...
    async duplicateAlumniStoryCategory(_: any, { input }: any, context: any) {
      try {
    ...
    async alumniStoriesActions(_: any, { input }: any, context: any) {
      try {
        const check = await db.query.alumniStory.findFirst({
          where: (alumniStory, { eq }) => and(eq(alumniStory.id, input.ID)),
        });
