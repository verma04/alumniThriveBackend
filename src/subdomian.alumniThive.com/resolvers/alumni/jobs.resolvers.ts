  ...
  Query: {
    ...
    async getJobPostedByMe(_: any, {}: any, context: any) {
      try {
        ...
        const allJobs = await db.query.jobs.findMany({
          where: and(eq(jobs.postedBy, id), eq(jobs.organization, org_id)),
          with: {
            postedBy: {
              with: {
                alumni: {
    ...
    async getJobBySlug(_: any, { input }: any, context: any) {
      try {
        ...
        const job = await db.query.jobs.findFirst({
          where: and(eq(jobs.slug, input.slug), eq(jobs.organization, org_id)),
          with: {
            postedBy: {
              with: {
                alumni: {
    ...
    async postJob(_: any, { input }: any, context: any) {
      try {
        ...
        const addedJobs = await db
          .insert(jobs)
          .values({ ...input, organization: org_id, postedBy: id, slug })
          .returning();
