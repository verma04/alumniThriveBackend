      ...
      try {
        ...
        const re = excludeUser.map((set) => ({
          ...
          isConnectIonRequested: {
            isFollower: set?.requestSent?.find((set) => set?.senderId === id)
              ?.isAccepted,
  ...
  Mutation: {
    async connectToUser(_: any, { input }: connectInput, context: any) {
      try {
        ...
        const isReadyFollowingUser = await db.query.alumniConnection.findFirst({
          where: and(
    ...
    async acceptConnectionRequest(
      ...
      try {
        ...
        if (!input.accept) {
          await db.delete(alumniRequest).where(eq(alumniRequest.id, find.id));
        }
